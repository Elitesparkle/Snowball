...
class Help(commands.Cog):
    ...
    class HelpView(discord.ui.View):
        def __init__(self, bot, outer) -> None:
            self.bot: MyBot = bot
        ...
        async def utility(self, button, interaction) -> None:
            await self.on_interaction(button, interaction)

        @discord.ui.button(
            label="Draft", custom_id="Help¦Draft", style=discord.ButtonStyle.blurple
        )
        async def draft(self, button, interaction) -> None:
            await self.on_interaction(button, interaction)
        ...
        async def creative(self, button, interaction) -> None:
            await self.on_interaction(button, interaction)

        @discord.ui.button(
            label="Matchup", custom_id="Help¦Matchup", style=discord.ButtonStyle.blurple
        )
        async def matchup(self, button, interaction) -> None:
            await self.on_interaction(button, interaction)
    ...
    async def craft_embed(self, servers: int, page: str | None = None) -> discord.Embed:
        ...
        pages = {
            ...
            "Creative": ["/artwork", "/joke", "/spray <hero>", "/quiz [type]"],
            "Matchup": [
        ...
        for entry in pages[page]:
            ...
            full_command_name, options = [
                result.strip() if result else result for result in results
            ]

            command = self.bot.get_application_command(full_command_name)
            ...
            embed.add_field(name=syntax, value=command.description, inline=inline)
    ...
    @commands.slash_command(
        name="help", description="Get a list of commands available."
    )
    async def help(self, context: discord.ApplicationContext) -> None:
